...
def get_gaze_average_ratio(faces, print_text='left'):
    count = 0
    sum_gaze_ratio = 0
    # while count < 120:  # (10*30)*3=120 10:移行時間
    #     _, frame = cap.read()
...
default_ratio = [1.2, 1.7]
left_gaze_average_ratio = default_ratio[0]
right_gaze_avarage_ratio = default_ratio[1]
...
if shatter != 0:
    while True:  # (10*30)*3=120 10:移行時間
        ...
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        faces = detector(gray)
        left_gaze_average_ratio = get_gaze_average_ratio(
            faces=faces, print_text="left")
...
shatter = int(input())
if shatter != 0:
    right_gaze_avarage_ratio = get_gaze_average_ratio("right")
...
while True:
    ...
    for face in faces:
        ...
        if gaze_ratio <= left_gaze_average_ratio:
            cv2.putText(frame, "left",
                        (50, 100), font, 2, (0, 0, 255), 3)
        elif left_gaze_average_ratio < gaze_ratio < right_gaze_avarage_ratio:
            cv2.putText(frame, "center",
                        (50, 100), font, 2, (0, 0, 255), 3)
        else:
            cv2.putText(frame, "right",
                        (50, 100), font, 2, (0, 0, 255), 3)
        cv2.putText(frame, str(gaze_ratio_left_eye),
                    (50, 150), font, 2, (0, 0, 255), 3)
